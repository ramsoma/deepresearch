...
def mock_env_vars():
    ...
    with patch.dict(
        os.environ, {"LLM_MODEL": "gemini-pro", "TAVILY_API_KEY": "test-key"}
    ):
        yield
        ...
        "results": [
            {
                ...
                "score": 0.9,
                "metadata": {"authors": ["Test Author"], "date": "2024-01-01"},
            }
        ]
    }
    return mock_client
...
def test_minimal_report_generation(lead_researcher, test_config):
    ...
    for section in report.sections:
        ...
        if "[" in section.content and "]" in section.content:
            citations = section.content[
                section.content.find("[") : section.content.find("]") + 1
            ]
...
def test_report_to_markdown(lead_researcher, test_config):
    ...
    for section in report.sections:
        ...
        if section.title == "References":
            ...
            assert (
                section.content in md
            ), f"Section {section.title} content not found in markdown"
