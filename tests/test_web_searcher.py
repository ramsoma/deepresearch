...
class TestWebSearcher(unittest.TestCase):
    ...
    def test_metadata_extraction_and_url_validation(self):
            ...
            "results": [
                {
                    ...
                    "score": 0.9,
                    "metadata": {"authors": ["John Doe"], "date": "2023-05-15"},
                },
                {
        ...
        self.assertEqual(
            len(processed_results), 2
        )  # Only valid and no-url articles should be included
        # Valid URL
        ...
        self.assertEqual(
            no_url["authors"], ["Unknown Author"]
        )  # Domain fallback fails, so Unknown Author
