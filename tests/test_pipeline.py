...
from deep_research_agent.agents.lead_researcher import (
    LeadResearcher, ResearchReport, ReportSection,
    ResearchPlan, ContentAnalysis, ReviewFeedback
)
...
class TestPipeline(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_responses = {
            'plan': {
                ...
                "key_questions": ["What are the latest breakthroughs?", "How close are we to practical applications?"],
                "required_sources": {
            ...
            'analysis': {
                "main_points": ["Quantum computing uses qubits", "Recent breakthroughs in error correction"],
                "key_insights": ["Quantum supremacy achieved", "Practical applications emerging"],
                "technical_details": ["Qubit coherence times improving", "Error correction codes advancing"],
                "applications": ["Cryptography", "Drug discovery"],
                "future_directions": ["Fault-tolerant quantum computers", "Quantum internet"],
                "confidence_score": 0.9
            },
            'review': {
                "strengths": ["Clear explanation of basics", "Good coverage of recent developments"],
                "weaknesses": ["Could use more examples", "Technical details could be deeper"],
                "missing_elements": ["More examples", "Deeper technical details"],
                "suggestions": ["Add more real-world examples", "Include more technical details"],
                "confidence_score": 0.85,
                "priority_fixes": ["Add more examples", "Deepen technical details"]
            },
            'section': {
                "title": "Introduction",
                "content": "Quantum computing represents a revolutionary approach to computation, leveraging the principles of quantum mechanics to process information in fundamentally new ways. Recent developments have brought us closer to practical applications, with significant breakthroughs in error correction and quantum supremacy demonstrations.",
                "key_points": ["Quantum computing basics", "Recent breakthroughs", "Future potential"],
                "citations": [
                    {
                        "key": "smith2023",
                        "text": "Smith et al. (2023)",
                        "reference": "Smith, J., et al. (2023). Quantum Computing Advances. Nature, 123(4), 567-589.",
                        "url": "https://example.com/smith2023"
                    },
                    {
                        "key": "jones2021",
                        "text": "Jones (2021)",
                        "reference": "Jones, R. (2021). The Future of Quantum Computing. Science, 456(7), 890-912.",
        ...
        def mock_generate_content(prompt):
            ...
            if "research plan" in prompt.lower():
                ...
                response.text = json.dumps(self.mock_responses['section'])  # Default response
            return response
        ...
        self.patches = {
            'llm': patch('deep_research_agent.agents.lead_researcher.genai.GenerativeModel'),
            'web_searcher': patch('deep_research_agent.agents.subagents.web_searcher.WebSearcher'),
            'tavily': patch('deep_research_agent.agents.subagents.web_searcher.TavilyClient'),
            'reviewer': patch('deep_research_agent.agents.subagents.reviewer_agent.ReviewerAgent'),
            'section_generator': patch('deep_research_agent.agents.lead_researcher.SectionGenerator'),
            'citation_agent': patch('deep_research_agent.agents.lead_researcher.CitationAgent')
        }
        ...
        self.mocks['web_searcher'].return_value.search.return_value = self.mock_search_response
        ...
        self.mocks['section_generator'].return_value.forward.side_effect = mock_section_generator
        ...
        self.mocks['citation_agent'].return_value.forward.side_effect = mock_citation_agent
    ...
    def test_full_pipeline(self):
        ...
        for section in report.sections:
            self.assertTrue(len(section.content) > 0, f"Section {section.title} has no content")

        # Check for citations in the content
        has_citations = any("[" in section.content or "(" in section.content for section in report.sections)
