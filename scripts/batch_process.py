        ...
        for query_config in self.config["queries"]:
            ...
            report_path, artifacts_path = generate_report(
                args=type('Args', (), {
                    ...
                    'output': str(self.reports_dir / f"report_{query[:30]}_{self.timestamp}.md"),
                    'citation_style': query_config.get("citation_style", "chicago"),
                    'citation_threshold': query_config.get("citation_threshold", 0.6),
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Batch process multiple research queries')
