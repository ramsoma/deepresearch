...
class ExistingEvaluator:
    def __init__(self, batch_dir: str):
        ...
        self.artifact_evaluator = ArtifactEvaluator(artifacts_dir=str(self.artifacts_dir))
                ...
                for citation in section['citations']:
                    ...
                    if isinstance(citation, dict):
                        if 'reference' in citation:
                            ...
                            sections.append(f"- {citation['text']}: {citation['source']}")
                        else:
    ...
    def _evaluate_report(self, report_path: str, artifacts_path: str, ground_truth: str) -> Dict[str, Any]:
    ...
    def evaluate_reports(self, ground_truth_file: str) -> None:
        ...
        for report_path in self.reports_dir.glob("*.md"):
            ...
            # Try to find a matching ground truth by checking if the query is a substring
            ...
            evaluation = self._evaluate_report(str(report_path), str(artifacts_path), ground_truth)
    ...
    def _generate_evaluation_report(self, results: List[Dict[str, Any]]) -> None:
        ...
        logger.info(f"Detailed results saved to: {self.evaluations_dir / 'detailed_results.csv'}")

def main():
    """Main entry point for evaluating existing reports."""
    import argparse
    parser = argparse.ArgumentParser(description='Evaluate existing reports in a batch output directory')
