...
class LLMCitationAgent:
    ...
    def __init__(self, llm=None, citation_style: str = "chicago"):
        ...
        logger.info(
            f"Initialized LLMCitationAgent with citation style: {citation_style}"
        )

    def cite_text(
        self, text: str, sources: List[Dict[str, Any]], style: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        sources_md = "\n".join(
            [
                f"{i+1}. {', '.join(s.get('authors', ['Unknown Author']))} ({s.get('year', 'n.d.')}). {s.get('title', 'Untitled')}. {s.get('url', '[No URL available]')}"  # noqa
        ...
        prompt = f"""
You are a research assistant specializing in academic citations.
Your task is to insert in-text citations into the following text using
ONLY the provided sources. Follow these rules EXACTLY:
...
"""
        ...
        for i, s in enumerate(sources):
            ...
            citations.append(
                {"id": str(i + 1), "text": text, "reference": reference, "url": url}
            )
