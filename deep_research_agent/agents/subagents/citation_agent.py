...
class CitationAgent(BaseAgent):
    ...
    def forward(self, 
        ...
        referenced_citations = [self.citation_db[k] for k in self.referenced_citation_keys]
    ...
    def _add_citations_to_content(self, content: str) -> str:
        """Add citations to content using the citation database and only include referenced citations."""
        ...
        for sentence in sentences:
            ...
            for key, citation in self.citation_db.items():
                if calculate_similarity(sentence, citation.content) >= self.similarity_threshold:
    ...
    def _format_citation_group(self, citations: List[Citation]) -> str:
        ...
        sorted_citations = sorted(citations, key=lambda x: list(self.citation_db.keys()).index(x.citation_key))
    ...
    def _generate_reference_list(self) -> List[str]:
        ...
        sorted_citations = sorted(self.citation_db.values(), 
                                key=lambda x: list(self.citation_db.keys()).index(x.citation_key))
        # Format references with numbers
        return [f"[{i+1}] {citation.reference}" for i, citation in enumerate(sorted_citations)]
    ...
    def _format_reference(self, title: str, authors: List[str], year: int, url: str) -> str:
    ...
    def add_citation(self, citation: Dict[str, Any]) -> str:
