...
class ReviewFeedback(dspy.Signature):
    ...
    missing_elements: List[str] = dspy.OutputField(
        desc="Important elements that are missing"
    )
    ...
    priority_fixes: List[str] = dspy.OutputField(
        desc="High-priority items that should be addressed first"
    )
...
class ReviewerAgent(dspy.Module):
    ...
    def forward(
        ...
        logger.info(
            f"""Reviewing report with content:
                {report_content[:500] if report_content else 'No content'}..."""
        )

        prompt = f"""You are a research report reviewer. Your task is to evaluate
          the following research report and provide detailed feedback.
...
6. Citation and Reference Quality
   ...
   - Citations should only appear in the text where referenced,
      not as lists at the end of sections
   - All citations should be collated in a single References
...
Remember: Respond with ONLY the JSON object, no other text."""
        ...
        try:
            ...
            logger.info(
                f"Successfully parsed JSON response with {len(result.get('weaknesses', []))} weaknesses and {len(result.get('suggestions', []))} suggestions"  # noqa
            )
            ...
            logger.info(
                f"Generated feedback with confidence score: {feedback.confidence_score}"
            )
