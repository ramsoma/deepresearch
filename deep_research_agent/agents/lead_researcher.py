...
from ..prompts.research_prompts import ResearchPrompt
from .subagents.web_searcher import WebSearcher
from .subagents.citation_agent import CitationAgent
...
class ResearchStrategy(dspy.Signature):
    ...
    approach: str = dspy.InputField(desc="Research approach (breadth-first or depth-first)")
...
class ResearchPlan(dspy.Signature):
    ...
    subtopics: Dict[str, List[str]] = dspy.OutputField(desc="Subtopics for each main topic")
    key_questions: List[str] = dspy.OutputField(desc="Key questions to answer")
    required_sources: Dict[str, int] = dspy.OutputField(desc="Number of sources needed per topic")
...
class ContentAnalysis(dspy.Signature):
    ...
    technical_details: List[str] = dspy.OutputField(desc="Technical details and specifications")
    applications: List[str] = dspy.OutputField(desc="Applications and use cases")
    future_directions: List[str] = dspy.OutputField(desc="Future directions and developments")
    confidence_score: float = dspy.OutputField(desc="Confidence score (0-1)")
    citations: List[Dict[str, Any]] = dspy.OutputField(desc="Citations and references used in the analysis")
...
class ContentAnalyzer(dspy.Module):
    ...
    def forward(self, content: str) -> ContentAnalysis:
        ...
        prompt = f"""You are a content analyzer. Your task is to analyze the following content and extract key insights in JSON format.
...
class SectionGenerator(dspy.Module):
    ...
    def forward(self, topic: str, analysis: Dict[str, Any], section_type: str = None) -> ReportSection:
...
class LeadResearcher(dspy.Module):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.citation_agent = LLMCitationAgent(citation_style=self.config.get("citation_style", "chicago"))
    ...
    def forward(self, query: str, strategy: Dict[str, Any]) -> ResearchReport:
        ...
        try:
            ...
            for topic in plan.main_topics:
                ...
                content_prompt = f"""Generate detailed content about the following topic in the context of the research query.
...
The content should be detailed and informative, suitable for a research report."""
            ...
            initial_report = self._generate_report(query, strategy, plan, analyses, search_results)
            ...
            citation_issues = [w for w in review_feedback.weaknesses if any(term in w.lower() for term in ['citation', 'reference', 'cite'])]
            if citation_issues:
                logger.warning(f"Citation-related issues found in review: {citation_issues}")
            
            # If there are significant issues, regenerate the report
            if review_feedback.confidence_score < 0.7 or len(review_feedback.priority_fixes) > 0:
                logger.info(f"Regenerating report based on review feedback. Confidence score: {review_feedback.confidence_score}, Priority fixes: {review_feedback.priority_fixes}")
                ...
                final_citation_issues = [w for w in final_review.weaknesses if any(term in w.lower() for term in ['citation', 'reference', 'cite'])]
                if final_citation_issues:
                    logger.warning(f"Citation issues still present after regeneration: {final_citation_issues}")
                else:
                    logger.info("All citation issues were resolved in the improved report")
    ...
    def _generate_report(self, query: str, strategy: Dict[str, Any], plan: ResearchPlan, 
                        analyses: Dict[str, ContentAnalysis], 
                        search_results: List[dict],
                        review_feedback: Optional[ReviewFeedback] = None) -> ResearchReport:
        ...
        for section_type in [s for s in sections_list if s != "References"]:
            ...
            analysis = analyses.get(section_type.lower(), {})
        ...
        if section_texts and search_results:
            # Combine all text
            logging.info(f"Creating citations for..")
            combined_text = "\n\n".join(section_texts)
            ...
            logger.info(f"Added References section with {len(citation_result['citations'])} citations")
        ...
        report = ResearchReport(
            ...
            metadata={
                ...
                "review_feedback": self._convert_to_dict(review_feedback) if review_feedback else None,
                "total_citations": len(citation_result["citations"]) if section_texts and sources else 0,
                "citation_style": self.config.get("citation_style", "chicago"),
                "citation_metrics": {
                    "total_citations": len(citation_result["citations"]) if section_texts and sources else 0,
                    "unique_citations": len(set(c["text"] for c in citation_result["citations"])) if section_texts and sources else 0,
                    "sections_with_citations": len([s for s in sections if s.citations])
                }
            }
        )
    ...
    def _generate_title(self, query: str) -> str:
        ...
        try:
            ...
            prompt = f"""Generate a clear, concise title for a research report about: {query}
...
Respond with ONLY the title, no additional text or explanation."""
    ...
    def _update_sections_based_on_feedback(self, required_sections: List[str], review_feedback: Dict) -> List[str]:
        ...
        try:
            ...
            if "sections_to_remove" in review_feedback:
                required_sections = [s for s in required_sections if s not in review_feedback["sections_to_remove"]]
